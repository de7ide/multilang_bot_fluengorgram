button-pressed = You pressed the buttonsdfdf
photo-send = photo/eng.jpg
photo-caption =🧠 You’re not like everyone else — you’re here, and that already makes you special. Ready to change your life?
                🔥 Find out how I make money — subscribe to my Telegram channel!
                💸 Want to take action right now? Click on the <b>“📖Schemes”</b> button and get yours
schemes = 📖 Schemes
my-chanal = 📣 My chanal
reviews = 💬 Reviews
schemes-pressed = Select available schemes:
ai-analisys = AI Analysis🤖
cheat-luck = 🎲 Cheat Luck
tic-tac-toe = ❌ Tic Tac Toe
back-first = 🔙 Back
ai-analisys-pressed = Select available actions:
schema = 📄 Scheme
ai-analisys2 = 🤖AI Analysis
back-sec = 🔙Back
schema-pressed = https://telegra.ph/AI-Analiz-11-08
send-id = Send ID
successfully = Successfully ✅
analysis = Analysis...
pick-up = Pick up at 1.
chance = Chance
back-menu-f = You are back to the previous menu...
schema2 = https://telegra.ph/Luck-Bait-11-08
choose-action = Select action...
my-chanal2 = 📣My chanal
rev = t.me/+QyxEwUJsxys4YWIy
r